...
export class LineComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    d3.json(
      ...
      const result = JSON.parse(JSON.stringify(chartData)).data.reduce(
      ...
      this.setup(Object.entries(barChartData));
      this.updateGraphData(Object.entries(barChartData));
    });

    // this.updateGraphData(barChartData);
  }

  /**
   * Kick off all initialization processes
   */
  private setup(barChartData): void {
    // console.log(barChartData);
    // console.log('LineChartComponent:setup');
    this.chartData.data = barChartData;
  ...
  private buildSvg(): void {
    ...
    let svgElement: any =
      this.htmlElement.getElementsByClassName('svg-chart')[0];
   ...
   * @param obsData
   */
  public updateGraphData(barChartData): void {
  ...
  private configureYaxis(): void {
    ...
    this.y = d3Scale.scaleLinear().range([this.height, 0]).domain(yRange);
